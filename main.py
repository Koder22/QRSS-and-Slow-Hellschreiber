...
slowheight = 10000 #10000pwm = 20Hz
slowlength = 0.5 #500ms per dot

pwm = PWM (Pin(10)) # start PWM on pin 10
pwm.freq (1000) # set PWM frequency at 100Hz
pwm.duty_u16 (0) # duty 0% (65535/2)
start = Pin(11, Pin.OUT) #define TX-start pin

start.value(1) #stop transmitter
...
for i in range(len(callsign)): #print callsign with slowhell
    ...
    for x in range(34):
        if '1' in buffer[x]:
            ...
            pwm.duty_u16 (0)
            start.value(1) #stopp transmitter
...
start.value(0) #start transmitter to send low line before first
